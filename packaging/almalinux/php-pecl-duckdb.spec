# AlmaLinux spec file for php-pecl-duckdb, built against the Remi
# repository's PHP (the default-namespace php packages, enabled via the
# remi-php82/83/84/85 repos — NOT the phpXX-php-* SCL variants).
#
...
# /etc/php.d exactly like Fedora/RHEL AppStream PHP.
...
# The php(api)/php(zend-abi) provides are shared by Remi's php-common and
# AppStream php-common, so the built package installs on any PHP with a
# matching ABI — e.g. an rpm built against Remi 8.4 also satisfies
# AppStream php 8.4 where that exists.
...
because AlmaLinux/EPEL does not package DuckDB. It is built against the
Remi repository's PHP and follows the Fedora/Remi PHP extension
conventions (php-pecl-* naming, /etc/php.d ini drop-in, php(api) ABI
pinning).
...
- Initial AlmaLinux package (Remi PHP target).
